import React from "react";
import { Component } from "../../components/Component";
import { Group } from "../../components/Group";
import { GroupWrapper } from "../../components/GroupWrapper";
import "./style.css";

export const DesktopLightMode = () => {
  const scrollToSection = (sectionId) => {
    const sectionElement = document.querySelector(`.${sectionId}`);
    if (sectionElement) {
      sectionElement.scrollIntoView({ behavior: "smooth" });
    }
  };

  return (
    <div className="desktop-light-mode">
      <div className="div-2">
        <div className="group-3">
          <div className="navbar">
            <button onClick={() => scrollToSection("text-wrapper-6")} className="text-wrapper-2">
              Tech Stack
            </button>
            <button onClick={() => scrollToSection("text-wrapper-20")} className="text-wrapper">
              Projects
            </button>
          </div>
          <div className="group-4">
            <a href="https://www.linkedin.com/in/esteban-baigts-70b423232/" target="_blank" rel="noopener noreferrer">
              <img
                className="linkedin"
                alt="Entypo social"
                src="https://c.animaapp.com/LcNIGwu7/img/entypo-social-linkedin-with-circle.svg"
              />
            </a>
            <a href="https://github.com/estebanbaigts" target="_blank" rel="noopener noreferrer">
              <img
                className="github"
                alt="Vector"
                src="https://c.animaapp.com/LcNIGwu7/img/vector-4.svg"
              />
            </a>
          </div>

          <img className="datacyb" alt="Datacyb" src="https://c.animaapp.com/LcNIGwu7/img/datacyb@2x.png" />
        </div>
        <div className="text-wrapper-6">My Tech Stack</div>
        <div className="text-wrapper-20">Projects</div>
        <img
          className="akar-icons-github"
          alt="Akar icons github"
          src="https://c.animaapp.com/LcNIGwu7/img/akar-icons-github-fill-4.svg"
        />
        <img
          className="logos-git-icon"
          alt="Logos git icon"
          src="https://c.animaapp.com/LcNIGwu7/img/logos-git-icon.svg"
        />
        <img className="logos-react" alt="Logos react" src="https://c.animaapp.com/LcNIGwu7/img/logos-react.svg" />
        <img
          className="vscode-icons-file"
          alt="Vscode icons file"
          src="https://c.animaapp.com/LcNIGwu7/img/vscode-icons-file-type-js-official.svg"
        />
        <div className="overlap-group">
          <div className="rectangle" />
          <div className="text-wrapper-8">
            <a href="https://github.com/estebanbaigts/N4S" target="_blank" rel="noopener noreferrer">
              View Code
            </a>
          </div>
          <div className="text-wrapper-9">Projet 🔨 - N4S</div>
          <p className="p">
            A project based on AI, the goal being to connect an AI to a small car with lasers that allows it to orient
            itself and we had to make it move forward without it hitting a wall
          </p>
          <p className="tech-stack-c">
            <span className="span">Tech stack :</span>
            <span className="text-wrapper-10">&nbsp;</span>
            <span className="text-wrapper-11">C</span>
          </p>

          <img
            className="img"
            alt="Akar icons github"
            src="https://c.animaapp.com/LcNIGwu7/img/akar-icons-github-fill-3.svg"
          />
          <img
            className="DALLE"
            alt="Dalle"
            src="https://c.animaapp.com/LcNIGwu7/img/dall-e-2023-11-14-11-02-1@2x.png"
          />
        </div>
        <div className="overlap">
          <div className="text-wrapper-8">
            <a href="https://github.com/estebanbaigts/Popeye" target="_blank" rel="noopener noreferrer">
              View Code
            </a>
          </div>
          <div className="text-wrapper-14">Projet 🔨 - POPEYE</div>
          <p className="text-wrapper-15">
            The goal was to containerize and define the deployment of a simple web survey application.
          </p>
          <p className="tech-stack-HTML">
            <span className="span">Tech stack :</span>
            <span className="text-wrapper-10">&nbsp;</span>
            <span className="text-wrapper-11">HTML , JavaScript, CSS,, Docker</span>
          </p>
          <img
            className="akar-icons-github-2"
            alt="Akar icons github"
            src="https://c.animaapp.com/LcNIGwu7/img/akar-icons-github-fill-2.svg"
          />
          <img
            className="DALLE-2"
            alt="Dalle"
            src="https://c.animaapp.com/LcNIGwu7/img/dall-e-2023-11-14-11-09-19---a-basc-image-simple-web-survey-appl@2x.png"
          />
        </div>
        <div className="overlap-2">
          <div className="text-wrapper-8">
            <a href="https://github.com/estebanbaigts/RPG" target="_blank" rel="noopener noreferrer">
              View Code
            </a>
          </div>
          <div className="text-wrapper-16">Projet 🔨 - RPG</div>
          <div className="overlap-3">
            <p className="text-wrapper-17">
              Projet Epitech qui consiste à créer un jeux RPG en C avec la librairie CSFML
            </p>
            <p className="div-3">
              <span className="span">Tech stack :</span>
              <span className="text-wrapper-10">&nbsp;</span>
              <span className="text-wrapper-11">C</span>
            </p>
          </div>
          <img
            className="akar-icons-github-2"
            alt="Akar icons github"
            src="https://c.animaapp.com/LcNIGwu7/img/akar-icons-github-fill-1.svg"
          />
          <img
            className="img-2"
            alt="Adobestock"
            src="https://c.animaapp.com/LcNIGwu7/img/adobestock-546101577-preview-1@2x.png"
          />
        </div>
        <div className="overlap-4">
          <div className="text-wrapper-8">
            <a href="https://github.com/estebanbaigts/GreenMeet" target="_blank" rel="noopener noreferrer">
              View Code
            </a>
          </div>
          <div className="text-wrapper-18">Projet 🔨 - GreenMett</div>
          <div className="overlap-3">
            <p className="text-wrapper-17">
              Notre mission est de créer un réseau dynamique et vivant de jardins communautaires, favorisant la
              collaboration, la convivialité et la découverte.
            </p>
            <p className="div-3">
              <span className="span">Tech stack :</span>
              <span className="text-wrapper-10">&nbsp;</span>
              <span className="text-wrapper-11">HTML, CSS , JavaScript, React Native</span>
            </p>
          </div>
          <img
            className="akar-icons-github-2"
            alt="Akar icons github"
            src="https://c.animaapp.com/LcNIGwu7/img/akar-icons-github-fill.svg"
          />
          <img
            className="img-2"
            alt="Dalle"
            src="https://c.animaapp.com/LcNIGwu7/img/dall-e-2023-11-14-11-04-1@2x.png"
          />
        </div>
        <div className="overlap-5">
          <Component className="component-1" />
          <img
            className="esteban-photo"
            alt="Esteban photo"
            src="https://c.animaapp.com/LcNIGwu7/img/esteban-photo@2x.png"
          />
        </div>
        <div className="text-wrapper-19">Technologies that i use</div>
        <div className="that-i-like-to-do">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;What i did</div>
        <div className="text-wrapper-20">Projects</div>
        <div className="overlap-6">
          <div className="menu">
            <div className="about">Projects</div>
            <div className="about-2">Contact</div>
            <div className="about-3">Tech Stack</div>
          </div>
        </div>
        <img className="divider" alt="Divider" src="https://c.animaapp.com/LcNIGwu7/img/divider.svg" />
        <Group className="group-6" />
        <GroupWrapper className="group-7" />
        <img className="group-8" alt="Group" src="https://c.animaapp.com/LcNIGwu7/img/group@2x.png" />
        <img className="figma" alt="Figma" src="https://c.animaapp.com/LcNIGwu7/img/figma.svg" />
        <img className="vector-2" alt="Vector" src="https://c.animaapp.com/LcNIGwu7/img/vector-3.svg" />
        <div className="vector-wrapper">
          <img className="vector-3" alt="Vector" src="https://c.animaapp.com/LcNIGwu7/img/vector-1.svg" />
        </div>
        <img className="image" alt="Image" src="https://c.animaapp.com/LcNIGwu7/img/image-17.svg" />
        <img className="vector-4" alt="Vector" src="https://c.animaapp.com/LcNIGwu7/img/vector.svg" />
        <img className="group-9" alt="Group" src="https://c.animaapp.com/LcNIGwu7/img/group-12.svg" />
        <img className="group-10" alt="Group" src="https://c.animaapp.com/LcNIGwu7/img/group-13.svg" />
      </div>
    </div>
  );
};
